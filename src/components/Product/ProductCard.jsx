  ...
  return (
    ...
    {/* Wishlist Icon */}
      <div className="absolute top-2 right-2 z-10">
        <button onClick={()=>{addToWishList(product)}} className="text-gray-500 hover:text-red-500 transition">
          <FaHeart />
        </button>
      </div>
      <motion.img
        src={product.image}
        alt={product.name}
        className={`${isList ? 'w-40 h-40 object-cover rounded-l-lg' : 'w-full h-56 object-cover rounded-t-lg'}`}
